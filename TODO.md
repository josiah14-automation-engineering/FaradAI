...
- ~~**[#44] Add bats unit tests for validation and flag-parsing logic**~~ ✓ resolved — 40 tests in `test/unit.bats` covering `_validate_memory/cpus/pids/network_mode`, `_build_extra_docker_args` allowlist, and `-n`/`-a` flag parser. Docker mocked via `test/helpers/docker`. `test/libs/` gitignored; CI clones bats-core. See BUILDLOG Session 33.
